...
test('portal deployment template includes identity and package bootstrap inputs', () => {
  ...
  assert.match(template.parameters.brokerPackageUri.defaultValue, /releases\/download\/v0\.3\.0-broker\/released-package\.zip\?download=1$/);
  assert.match(template.parameters.adminPackageUri.defaultValue, /releases\/download\/v0\.3\.0-admin\/released-package\.zip\?download=1$/);
});
