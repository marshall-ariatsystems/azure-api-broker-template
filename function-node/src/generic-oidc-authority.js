...
const { evaluateAssurance } = require('./assurance-policy');
...
function evaluateConfiguredAssurance(claims, assurance) {
  if (!Object.keys(assurance).length) return;
  const { claim, equals } = assurance;
  if (typeof claim !== 'string' || !claim || typeof equals !== 'string' || !equals) throw new Error('required assurance is missing');
  const value = claims[claim];
  // Adapt the configured claim to the shared acr/amr contract.  Only arrays get
  // membership semantics; strings remain exact scalar values.
  const collection = Array.isArray(value);
  const result = evaluateAssurance(
    collection ? { acr: '__generic-oidc-assurance__', amr: value } : { acr: value, amr: ['__generic-oidc-assurance__'] },
    collection ? { requiredAcr: '__generic-oidc-assurance__', requiredAmr: equals } : { requiredAcr: equals, requiredAmr: '__generic-oidc-assurance__' },
  );
...
async function validateRequest(req, { environment = process.env, fetcher = fetch, now = Date.now() } = {}) {
  ...
  evaluateConfiguredAssurance(claims, assurance);
...
module.exports = { validateRequest, evaluateConfiguredAssurance };
